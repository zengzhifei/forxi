  ...
  <div class="min-h-screen bg-gray-100 text-zinc-600 font-sans flex flex-col">
    ...
    <main class="flex-1">
      <section class="relative overflow-hidden bg-white border-b border-zinc-100">
        <div class="absolute inset-0" style="background: linear-gradient(135deg, #dbeafe 0%, #ffffff 40%, #e0f2fe 100%);"></div>
        <div class="relative max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-16 text-center">
          ...
          <h1 class="text-2xl sm:text-5xl font-bold text-zinc-800 mb-2 sm:mb-4 tracking-tight">查询地理信息</h1>
          <p class="text-zinc-400 text-xs sm:text-lg max-w-2xl mx-auto leading-relaxed">查询IP地址的地理位置与实时天气</p>
        </div>
      </section>
              ...
              <div class="p-5 space-y-4">
                <div class="flex items-center justify-center py-4">
                  <div class="text-center">
                    <div class="text-4xl mb-2">{{ getCountryEmoji(ipData.geo.countryCode) }}</div>
                    <div class="text-lg font-semibold text-zinc-800">{{ ipData.geo.country }}</div>
                    <div class="text-sm text-zinc-500">{{ ipData.geo.countryCode }}</div>
                  </div>
                </div>
                <div class="h-px bg-zinc-100"></div>
                <div class="grid grid-cols-2 gap-4">
                  <div class="space-y-3">
                    <InfoCard label="查询状态" :value="ipData.geo.status === 'success' ? '成功' : '失败'" icon="status" />
                    <InfoCard label="区域" :value="ipData.geo.regionName || '-'" icon="map" />
                    <InfoCard label="经度" :value="ipData.geo.lon?.toFixed(4) || '-'" icon="lon" />
                    <InfoCard label="邮政编码" :value="ipData.geo.zip || '-'" icon="zip" />
                    <InfoCard label="时区" :value="ipData.geo.timezone || '-'" icon="timezone" />
                  </div>
                  <div class="space-y-3">
                    <InfoCard label="查询IP" :value="ipData.geo.query || '-'" icon="ip" copyable />
                    <InfoCard label="城市" :value="ipData.geo.city || '-'" icon="city" />
                    <InfoCard label="纬度" :value="ipData.geo.lat?.toFixed(4) || '-'" icon="lat" />
                    <InfoCard label="区域代码" :value="ipData.geo.region || '-'" icon="region" />
                    <InfoCard v-if="ipData.weather" label="当地时间" :value="getLocalTime(ipData.geo.timezone)" icon="clock" />
                  </div>
                </div>
              </div>
            </div>
                ...
                <div class="bg-zinc-50 rounded-xl p-4">
                  <div class="flex items-center gap-3 mb-3">
                    ...
                    <div>
                      ...
                      <div class="font-medium text-zinc-800">{{ ipData.geo.isp || '-' }}</div>
                    </div>
                  </div>
                  <div class="text-xs text-zinc-400 truncate">{{ ipData.geo.org || '-' }}</div>
                </div>
                <div class="h-px bg-zinc-100"></div>
                <div class="space-y-3">
                  <InfoCard label="组织/公司" :value="ipData.geo.org || '-'" icon="org" />
                  <InfoCard label="AS号码" :value="ipData.geo.as || '-'" icon="as" />
                  <InfoCard label="IP类型" :value="getIpType(ipData.geo.isp)" icon="type" />
                </div>
              </div>
            </div>
          </div>
            ...
            <div class="p-5">
              ...
              <div class="mt-5 pt-5 border-t border-zinc-100">
                <div class="flex flex-wrap gap-2">
                  <span class="px-3 py-1.5 bg-blue-50 text-blue-600 text-xs rounded-full">当地时间: {{ getLocalTime(ipData.geo.timezone) }}</span>
                  ...
                  <span class="px-3 py-1.5 bg-violet-50 text-violet-600 text-xs rounded-full">时区: {{ ipData.geo.timezone || '-' }}</span>
