<template>
  <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
    ...
    <div class="space-y-4">
      ...
      <div>
        <label class="block text-sm font-medium text-gray-700 mb-2">密钥</label>
        <input 
          type="text" 
          v-model="key" 
...
function encrypt() {
  if (!input.value || !key.value) {
    toast.warning('请输入文本和密钥')
    return
  }
  const shift = key.value.split('').reduce((a, b) => a + b.charCodeAt(0), 0) % 26
  output.value = caesarCipher(input.value, shift)
}
...
function decrypt() {
  ...
  const shift = key.value.split('').reduce((a, b) => a + b.charCodeAt(0), 0) % 26
  output.value = caesarCipher(input.value, 26 - shift)
}
