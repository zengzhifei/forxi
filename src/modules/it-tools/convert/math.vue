    ...
    <div class="space-y-4">
      <div>
        ...
        <input 
          type="text" 
          v-model="expression" 
          class="w-full p-3 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-zinc-300 focus:border-zinc-400 font-mono"
          placeholder="例如: 2+2, sin(3.14), sqrt(16)..."
          @keyup.enter="calculate"
        />
      </div>
      <button @click="calculate" class="px-4 py-2 bg-zinc-600 text-white rounded-lg hover:bg-zinc-700 transition-colors">计算</button>
      <div v-if="result !== null" class="p-4 bg-gray-50 rounded-lg">
        <label class="block text-sm font-medium text-gray-700 mb-2">结果：</label>
        <p class="text-2xl font-bold text-zinc-700">{{ result }}</p>
      </div>
